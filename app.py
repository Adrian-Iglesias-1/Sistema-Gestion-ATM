    ...
    if st.session_state.matches is not None:
        ...
        if len(matches_df) > 0:
            ...
            with col3:
                ...
                st.metric("Duración Promedio (hrs)", f"{avg_duration:.1f}")
            ...
            if len(date_range) == 2:
                ...
                filtered_df = filtered_df[
                    (filtered_df['Fecha_Orden'].dt.date >= start_date) &
                    (filtered_df['Fecha_Orden'].dt.date <= end_date)
                ]
            ...
            if len(filtered_df) > 0:
                ...
                display_df['Fecha_Orden'] = display_df['Fecha_Orden'].dt.strftime('%Y-%m-%d %H:%M')
                display_df['Inicio_Downtime'] = display_df['Inicio_Downtime'].dt.strftime('%Y-%m-%d %H:%M')
                display_df['Fin_Downtime'] = display_df['Fin_Downtime'].dt.strftime('%Y-%m-%d %H:%M')
                display_df['Duracion_Downtime_Horas'] = display_df['Duracion_Downtime_Horas'].round(2)
                display_df['Diferencia_Tiempo_Minutos'] = display_df['Diferencia_Tiempo_Minutos'].round(1)
